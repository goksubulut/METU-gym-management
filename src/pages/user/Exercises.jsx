...
const MUSCLE_TABS = [{ value: "Tümü", label: "Tümü" }, ...MUSCLE_GROUPS.map((g) => ({ value: g.id, label: g.label }))];
          ...
          {filtered.map((e) => (
            <Card key={e.id} soft onClick={() => nav(`/exercises/${e.id}`)} className="overflow-hidden">
              <div className="hero-sheen grid h-24 place-items-center bg-gray-900 bg-gradient-to-br from-ink-800 to-ink-950 text-white/85">
                <Icon name={TYPE_ICON[e.type] ?? "dumbbell"} size={30} strokeWidth={1.4} />
              </div>
              <div className="p-3">
          ...
          {filtered.map((e) => (
            <Card key={e.id} onClick={() => nav(`/exercises/${e.id}`)} className="flex items-center gap-3 p-3">
              <div className="grid h-12 w-12 shrink-0 place-items-center rounded-xl bg-gray-900 text-white">
                <Icon name={TYPE_ICON[e.type] ?? "dumbbell"} size={19} />
              </div>
