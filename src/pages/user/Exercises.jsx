          ...
          <div className="grid grid-cols-2 gap-3">
            {paged.map((e) => (
              <Card key={e.id} soft onClick={() => nav(`/exercises/${e.id}`)} className="overflow-hidden">
                <div className={`grid h-24 place-items-center ${TYPE_COLOR[e.type] ?? "bg-ink-900"} text-white`}>
          ...
          <div className="space-y-2">
            {paged.map((e) => (
              <Card key={e.id} onClick={() => nav(`/exercises/${e.id}`)} className="flex items-center gap-3 p-3">
                <div className={`grid h-11 w-11 shrink-0 place-items-center rounded-xl ${TYPE_COLOR[e.type] ?? "bg-ink-900"} text-white`}>
